import { POST } from '../posts'

export const naverwebtoonbookmarker = {
    pid: 'naverwebtoonbookmarker',
    title: '네이버 웹툰 북마크',
    thumbnail: './portfolio/bookmarker.avif',
    description: `
    일 마치고 컴퓨터에 앉아 밥을 먹으며 잠시 숨돌리던 그 시간, 웹툰을 찾는 시간도 아까워서 <br>
<br>
    "이렇게 매번 찾지말고 이거마저 내가 원하는 것만 띄워보자"<br>
    <br>
    라는 생각으로 만들게되었습니다<br>
    <br>
    저번부터 크롬 익스텐션을 이용한 무언가를 해보고 싶었으나 아이디어가 없어서 못하던 찰나<br>
    <br>
    마침 이 아이디어를 살려서 익스텐션 제작을 시작하게 되었습니다 <br>
    `,
    stack: ['react', 'crxjs', 'chakra ui'],
    github: 'https://github.com/B-HS/Naver_Webtoon_Bookmarker',
    link: 'https://chrome.google.com/webstore/detail/%EB%84%A4%EC%9D%B4%EB%B2%84-%EC%9B%B9%ED%88%B0-%EB%B6%81%EB%A7%88%ED%81%AC/cnfglfaobnkgkcnagfbaeilbpkcjcanh/related?hl=ko',
    from: new Date('2023-03-11 00:00:00.000000'),
    to: new Date('2023-03-12 00:00:00.000000'),
} as POST
